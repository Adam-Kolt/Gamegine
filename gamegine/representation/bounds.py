...
class DiscreteBoundary(Boundary, Drawable):
    ...
    def intersects_line(self, x1: SpatialMeasurement, y1: SpatialMeasurement, x2: SpatialMeasurement, y2: SpatialMeasurement) -> bool:
    ...
    def buffered(self, distance: SpatialMeasurement) -> 'DiscreteBoundary': # Efficiency is cooked here...but its easy
        self.__recompute_plain_points()
        coords = self.__convert_coordinate_sequence(sg.Polygon(self.plain_points).buffer(distance, quad_segs=1).exterior.coords)
        return Polygon(coords)


class Rectangle(DiscreteBoundary):
    def __init__(self, x: SpatialMeasurement, y: SpatialMeasurement, width: SpatialMeasurement, height: SpatialMeasurement):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
...
class Circle(Boundary):
    def __init__(self, x: SpatialMeasurement, y: SpatialMeasurement, radius: SpatialMeasurement):
        self.x = x
        self.y = y
        self.radius = radius
...
class Line(DiscreteBoundary):
    def __init__(self, x1: SpatialMeasurement, y1: SpatialMeasurement, x2: SpatialMeasurement, y2: SpatialMeasurement):
        self.x1 = x1
        self.y1 = y1
        self.x2 = x2
        self.y2 = y2
...
class BoundedObject(NamedObject):
    ...
    def translated(self, x: SpatialMeasurement, y: SpatialMeasurement) -> 'BoundedObject':
        obj = copy.deepcopy(self)
        obj.bounds = obj.bounds.translate(x, y)
        return obj
    
    def translate_ip(self, x: SpatialMeasurement, y: SpatialMeasurement) -> 'BoundedObject':
...
def LineIntersectsAnyBound(bounds: List[DiscreteBoundary], x1: SpatialMeasurement, y1: SpatialMeasurement, x2: SpatialMeasurement, y2: SpatialMeasurement) -> bool:
    for bound in bounds:
...
def CircularPattern(objects: List[BoundedObject], center: Tuple[SpatialMeasurement, SpatialMeasurement], angle: AngularMeasurement, num_objects: int, prefix_func: Callable[[int], str]) -> List[BoundedObject]:
    if num_objects <= 1:
...
def SymmetricalX(objects: List[BoundedObject], axis: SpatialMeasurement, prefix: str, prefix_og: str = "") -> List[BoundedObject]:
    return [obj.prefix(prefix_og) for obj in objects] + [obj.mirrored_over_vertical(axis).prefix(prefix) for obj in objects]

def SymmetricalY(objects: List[BoundedObject], axis: SpatialMeasurement, prefix: str, prefix_og: str = "") -> List[BoundedObject]:
    return [obj.prefix(prefix_og) for obj in objects] + [obj.mirrored_over_horizontal(axis).prefix(prefix) for obj in objects]

def SymmetricalXY(objects: List[BoundedObject], axis_x: SpatialMeasurement, axis_y: SpatialMeasurement, prefix: str, prefix_og: str = "") -> List[BoundedObject]:
...
def ExpandedObjectBounds(objects: List[BoundedObject], robot_radius: SpatialMeasurement = Inch(21), discretization_quality=4) -> List[DiscreteBoundary]:
    return [object.bounds.discretized(discretization_quality).buffered(robot_radius) for object in objects]

def ExpandedBounds(bounds: List[Boundary], robot_radius: SpatialMeasurement = Inch(21), discretization_quality=4) -> List[DiscreteBoundary]:
